...
file = pd.read_csv ("stock_data/Dow_Jones/Dow_Jones_30.csv")
ticker_list=[]
...
##### save 
f=open('stock_data/Dow_Jones/Dow_Jones_10year_stock.txt', 'w')
f.write(tickers_more_than_10years)
f.close()

##### read
f=open('stock_data/Dow_Jones/Dow_Jones_10year_stock.txt', 'r')
...
start_date = data['UNH'].index[0].strftime('%Y/%m/%d') ###'1962/01/02'
start_year = int(start_date[:4])
start_month = int(start_date[5:7])
start_day = int(start_date[-2:])
# print(start_year, start_month, start_day)
end_date = data['UNH'].index[-1].strftime('%Y/%m/%d')  ###'2022/11/30'
...
datas = np.zeros((29, len(weekdays), 6))
labels = np.zeros((29,len(weekdays)))
j=0
...
### save raw data
np.save('stock_data/Dow_Jones/Dow_Jones_29_data.npy', datas)
np.save('stock_data/Dow_Jones/Dow_Jones_29_label.npy', labels)

#### load raw data
x = np.load('stock_data/Dow_Jones/Dow_Jones_29_data.npy')
x_max = np.nanmax(x,axis=1)
x_min = np.nanmin(x,axis=1)
x_max= np.tile(x_max[:,None,:], (1, 15892, 1))
x_min= np.tile(x_min[:,None,:], (1, 15892, 1))
