        ...
        with tqdm(train_loader, mininterval=5.0, maxinterval=5.0) as it:
            for batch_no, train_batch in enumerate(it, start=1):
                with tf.GradientTape() as tape:
                    noise, predicted, target_mask = model(train_batch)
...
class DiffusionEmbedding(keras.Model):
    def __init__(self, num_steps, embedding_dim=128, projection_dim=None):
        ...
        self.projection1 = keras.layers.Dense(projection_dim, kernel_initializer= 'he_uniform', bias_initializer= 'he_uniform')
        # nn.Linear(embedding_dim, projection_dim)
        self.projection2 = keras.layers.Dense(projection_dim, kernel_initializer= 'he_uniform', bias_initializer= 'he_uniform')
